...
public class GerarItemFundo : MonoBehaviour
{
    ...
    void OnEnable()
    {
        CriarItens("Background");
    }

    void OnDisable()
    {
        //laço para destroir a variavel de referencia da lista de background
    ...
    {
        //vetor com o tamanho da quantidade de registros de background
        x = new int[bd.QueryInt("count(id)", tabela)];
        //variavel é usado para referenciar todos os objetos lista de background
        obj = new GameObject[x.Length];
        for (int i = 0; i < x.Length; i++)
        {
            //index utilizado para a consulta bd
            int index = i + 1;
            //query para adiquirir o nome dos background
